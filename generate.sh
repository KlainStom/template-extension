...
read owner
lowercaseowner="$(echo "$owner" | tr '[:upper:]' '[:lower:]' | tr '-' '')"
...
sed -i "s|EXTENSION_REPO_OWNER|$owner|g" README.md build.gradle.kts settings.gradle.kts src/main/resources/extension.json
sed -i "s|EXTENSION_REPO_LOWER_OWNER|$lowercaseowner|g" README.md build.gradle.kts settings.gradle.kts src/main/resources/extension.json
...
mkdir src/main/java/com/github/"$lowercaseowner"/"$lowercasename"/
